...
const _kGoldLight = Color(0xFFFFF8DC);
const _kSilver = Color(0xFFC0C0C0);
const _kSilverLight = Color(0xFFF5F5F5);
const _kSilverDark = Color(0xFF808080);
const _kBronze = Color(0xFFCD7F32);
const _kBronzeLight = Color(0xFFFFF0E0);
const _kBronzeDark = Color(0xFF8B4513);
...
class _MeScreenState extends State<MeScreen>
      ...
      body: SafeArea(
        child: NestedScrollView(
          headerSliverBuilder: (context, innerBoxIsScrolled) => [
            SliverToBoxAdapter(
              child: _buildHeader(
                  context, isDark, profile, stats, locale, themeProvider),
            ),
          ],
          body: Column(
            children: [
              ...
              Expanded(
                child: TabBarView(
                  ...
                  children: [
                    // Tab 0: Profile
                    _buildProfileTab(
                        context, isDark, profile, auth, isGoogleUser),
                    // Tab 1: Stats
                    _buildStatsTab(context, isDark, stats, locale),
                    // Tab 2: Achievements & Medals
                    _buildAchievementsTab(context, isDark, stats, locale),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildStatsTab(
    ...
    return ListView(
      ...
      children: [
        // ── Stats Grid ─────────────────────────────────────
        SectionHeaderWidget(label: 'สถิติของฉัน'),
        const SizedBox(height: 10),
        GridView.count(
          crossAxisCount: 2,
          shrinkWrap: true,
          ...
          children: [
            ...
            _StatCard(
              isDark: isDark,
              emoji: '💰',
              value: _formatAmount(stats.totalSpent),
              label: 'ยอดรวมทั้งหมด',
        ...
        // ── Medal Collection ───────────────────────────────
        SectionHeaderWidget(label: 'คอลเลกชันเหรียญ'),
        const SizedBox(height: 10),
        Container(
          padding: const EdgeInsets.all(20),
          decoration: BoxDecoration(
        ...
        // ── Points Progress ────────────────────────────────
        SectionHeaderWidget(label: 'ระดับของฉัน'),
        const SizedBox(height: 10),
        _LevelCard(isDark: isDark, points: stats.totalPoints),
      ],
    );
  }
  ...
  Widget _buildAchievementsTab(
    ...
    final locked =
        stats.achievements.where((a) => !a.unlocked).toList();

    return ListView(
      padding: const EdgeInsets.fromLTRB(16, 8, 16, 32),
      children: [
        // Unlocked count banner
        Container(
          padding:
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      'ความสำเร็จ ${unlocked.length}/${stats.achievements.length}',
                      style: GoogleFonts.notoSansThai(
                        fontSize: 15,
                        fontWeight: FontWeight.w700,
                        color: isDark
                    ...
                    ClipRRect(
                      ...
                      child: LinearProgressIndicator(
                        value: stats.achievements.isEmpty
                            ? 0
                            : unlocked.length /
        ...
        if (unlocked.isNotEmpty) ...[
          SectionHeaderWidget(label: 'ปลดล็อกแล้ว ✅'),
          const SizedBox(height: 10),
        ...
        if (locked.isNotEmpty) ...[
          SectionHeaderWidget(label: 'ยังไม่ปลดล็อก 🔒'),
          const SizedBox(height: 10),
        ...
        if (unlocked.isEmpty && locked.isEmpty)
          Center(
            child: Padding(
              ...
              child: Text(
                'ยังไม่มีความสำเร็จ เริ่มสร้างบิลเลย!',
                style: GoogleFonts.notoSansThai(
                  fontSize: 14,
                  color: isDark
                      ? AppColors.textTertiaryDark
...
// ── Medal Collection ───────────────────────────────────────────
class _MedalCollection extends StatelessWidget {
  final String emoji;
  final int count;
  final String label;
  final Color color;
...
class _LevelCard extends StatelessWidget {
  ...
  const _LevelCard({required this.isDark, required this.points});
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = _levels.length - 1; i >= 0; i--) {
      if (points >= _levels[i].$1) {
        levelIdx = i;
        break;
      }
    }
    final current = _levels[levelIdx];
    final next = levelIdx < _levels.length - 1
        ? _levels[levelIdx + 1]
        : null;
    final progress = next != null
        ? ((points - current.$1) / (next.$1 - current.$1))
            .clamp(0.0, 1.0)
        : 1.0;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            AppColors.primary.withValues(alpha: isDark ? 0.3 : 0.08),
            AppColors.primary.withValues(alpha: isDark ? 0.1 : 0.02),
          ],
        ),
        borderRadius: BorderRadius.circular(AppRadii.lg),
        border: Border.all(
      ...
      child: Column(
        children: [
          Row(
            children: [
              Text(current.$3, style: const TextStyle(fontSize: 36)),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
...
class _AchievementCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final title = locale.t(achievement.titleKey);
    final desc = locale.t(achievement.descKey);
            ...
            child: Column(
              ...
              children: [
                ...
                if (unlocked && achievement.unlockedAt != null) ...[
                  const SizedBox(height: 4),
                  Text(
                    'ปลดล็อก ${_formatDate(achievement.unlockedAt!)}',
                    style: GoogleFonts.notoSansThai(
                      fontSize: 11,
                      color: AppColors.primary,
                      fontWeight: FontWeight.w500,
