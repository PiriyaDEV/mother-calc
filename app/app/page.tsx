...
export default function AppPage() {
  ...
  // Redirect to login if not logged in (only when Supabase is configured)
  useEffect(() => {
    if (!authLoading && configured && !user) {
      router.push("/login");
    }
  }, [user, authLoading, router, configured]);
  ...
  useEffect(() => {
    if (total !== prevTotal && prevTotal !== 0) {
      ...
      const t = setTimeout(() => setTotalAnimating(false), 600);
  ...
  // If Supabase configured but not logged in → redirect (handled by useEffect)
      ...
      {hydrated && bill && itemCount > 0 && (
        <div className="fixed bottom-16 left-0 right-0 z-30">
          <div className="max-w-lg mx-auto px-4 pb-1">
            <div
              className={`bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-lg px-4 py-2.5 flex items-center justify-between transition-all duration-300 ${
                totalAnimating
                  ? "scale-[1.02] shadow-xl border-[#4366f4]/30"
                  : "scale-100"
              }`}
            >
              ...
              <div className="text-right">
                <div className="flex items-center gap-1 justify-end">
                  {totalAnimating && (
                    <IoTrendingUp
                      size={12}
                      className="text-[#4366f4] animate-bounce"
                    />
                  )}
